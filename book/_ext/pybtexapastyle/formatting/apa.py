...
if six.PY2:
    ...
    def format_pages(text):
        ...
        pages = Text(Symbol("ndash")).join(re.split(dash_re, six.text_type(text)))
...
def apa_names(children, context, role, **kwargs):
    ...
    if len(persons) > 7:
        ...
        return join(sep=", ", last_sep=", … ")[formatted_names].format_data(context)
    else:
        formatted_names = [
            style.format_name(person, style.abbreviate_names) for person in persons
        ]
...
class APAStyle(BaseStyle):
    ...
    def format_author_or_editor_and_date(self, e):
        if "author" in e.persons and "editor" in e.persons:
            ...
            return sentence(sep=" ")[self.format_names("author"), join["(", date, ")"]]
        else:
            return sentence(sep=" ")[
                self.format_editor(e, as_sentence=False), join["(", date, ")"]
            ]
    ...
    def format_url(self, e):
        ...
        return words["URL:", href[field("url", raw=True), field("url", raw=True)]]
    ...
    def get_inbook_template(self, e):
        ...
        # Optional fields: volume, series, address, edition, month, note, key
        return toplevel[
            sentence(sep=" ")[self.format_names("author"), join["(", date, ")"]],
            self.format_title(e, "title"),
            sentence(sep=" ")[
    ...
    def get_techreport_template(self, e):
        ...
        return toplevel[
            sentence(sep=" ")[
                self.format_names("author", as_sentence=False), join["(", date, ")"]
            ],
            self.format_btitle(e, "title"),
