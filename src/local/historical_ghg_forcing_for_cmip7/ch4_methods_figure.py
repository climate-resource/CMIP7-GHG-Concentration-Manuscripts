...
MOSAIC = [
    ["timeseries", "timeseries", "interpolated-most", "interpolated-most", "gm"],
    [
        "timeseries",
        "timeseries",
        "interpolated-least",
    ...
    ["locations", "locations", "counts", "lat-grad-eof", "lat-grad-pc"],
    [
        "gm-ext-l",
        "gm-ext-r",
        "lat-grad-pc-ext-l",
    ...
    ["flying-carpet", "monthly", "monthly", "yearly-l", "yearly-r"],
]
...
def plot_flying_carpet(
    ...
    ax.set_zlabel(label_name(f"[{tmp.attrs['units']}]"), rotation=90, fontsize="small")
    ax.set_ylabel(r"latitude [$^{\circ}$N]", fontsize="small")
    ax.set_yticks([-45, 45])
...
def generate_ch4_methods_figure(
    outfile: Path,
    bundle_dir: Path,
    original_run_notebooks_dir: Path = DEFAULT_ORIGINAL_RUN_NOTEBOOKS_DIR,
    force_rerun: bool = False,
    ...
    plot_global_mean_extension(
        ...
        axes["gm-ext-r"],
        input_sources={
            # TODO: add Law Dome input here (with label that makes clear different latitude)
