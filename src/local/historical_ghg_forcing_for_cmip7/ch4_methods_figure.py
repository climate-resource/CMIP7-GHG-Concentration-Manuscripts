...
MOSAIC = [
    ...
    ["lat-grad-pc-emms", "lat-grad-pc-emms", "flying-carpet", "flying-carpet"],
    ["monthly", "monthly", "yearly-l", "yearly-r"],
]
"""Layout of the figure's panels

Four columns, the same four the N2O figure has,
because the columns have to be wide enough
to carry a panel's labels and its colour bar
and four is as many as the page's width will take.
This figure has a panel the N2O figure does not,
so it pays for it with a sixth row rather than with a fifth column:
the page limits the figure's width but not its height,
and a fifth column makes every panel in the figure narrower.

No panel here may span columns 1 and 2.
Column spans have to nest: a panel may span columns 0 and 1,
or columns 2 and 3, but a panel which spans columns 1 and 2
ties every column in the figure to every other one.
Together with the maps, whose aspect ratio is fixed,
that leaves the layout engine no way to satisfy every panel at once,
so it gives up and every panel lands
